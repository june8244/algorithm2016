...
/*****************************************************
int
myStrNCmp(const string& s1, const string& s2)
{
   unsigned n2 = s2.size();
   if (n2 == 0) return -1;
   unsigned n1 = s1.size();
...
// find all 'del' character in str and delete them.
// sweep range (pos ~ last) can be specified.
inline void strSweep
(string& str , const char del , size_t pos = 0 , size_t last = string::npos)
{
    while (pos != last) {
        pos = str.find_first_of(del , pos);
        if (pos != last) str.erase(pos,1);
    }
}
...
inline void tran_type(A& a,B& b)//a to b
{
  stringstream ss;
  ss<<a;
  ss>>b;
...
inline vector<size_t> scan(string data){
	vector<size_t> numbers;
	int temp=0, i=0, push=0;
	while(i<data.size()){
		temp=0; push=0;
